...
#include<string>
#include<algorithm>
std::string Pet[] = { "None", "Earthworm", "Titmouse", "Vulture", "Cat", "Frog", "Mosquito", "Snake", "Hedgehog", "Human", "Grasshopper" };

int a[11][4] = {
	{}, {1}, {1, 2}, {2}, {3}, {3}, {3, 4, 5}, {6, 4, 5}, {4}, {6, 7, 8, 10}, {9, 6, 8}
};
class Animal
{
private:
...
{
	std::cout << "The fighter for life " << st.name << " is " << st.power << " powerful " << Pet[st.type] << std::endl;
}
int naturalSelection(Animal* victim, Animal* aggressor)
{
	...
	{
		std::cout << (*victim).name << " the " << Pet[(*victim).type] << " was eaten by " << Pet[(*aggressor).type] << " named " << (*aggressor).name << std::endl;
	...
	{
		std::cout << (*aggressor).name << " the " << Pet[(*aggressor).type] << " was eaten by " << Pet[(*victim).type] << " named " << (*victim).name << std::endl;
...
void Fun(Animal** room, int first, int second, int n)
{
	...
	{
		std::cout << "The winner is " << (*room[n - 1]).getName() << " the " << Pet[(*room[n - 1]).getType()] << n;
	}
	else if (done == 1)
	{
		room[first] = room[--n];
